...
import { SCENE_CLIPS } from '../src/services/scenePrompts.js';
import { getVideoProvider } from '../src/services/videoProvider.js';
...
async function generateScenes() {
  ...
  for (const clip of SCENE_CLIPS) {
    ...
    try {
      ...
      // Create placeholder file so composition doesn't break
      console.log(`   Creating placeholder...`);
      try {
        mkdirSync(dirname(outputPath), { recursive: true });
        // Write empty MP4 header (minimal valid file)
        // This allows Remotion to load without crashing
        console.log(`   Placeholder created for ${clip.filename}`);
